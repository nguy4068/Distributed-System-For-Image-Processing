...
echo "ssh -t -n -f ramkr004@$NODE_0_IP  \"csh -c '$CA'\" > /dev/null"  > command0.sh
source command0.sh

#Start the compute node 1
CA=`echo "$setenvstring cd $PROJ_PATH &&  nohup $NODE_COMMAND_1 & "`
echo "ssh -t -n -f ramkr004@$NODE_1_IP  \"csh -c '$CA'\" > /dev/null"  > command1.sh
source command1.sh

#Start the compute node 2
CA=`echo "$setenvstring cd $PROJ_PATH &&  nohup $NODE_COMMAND_2 & "`
echo "ssh -t -n -f ramkr004@$NODE_2_IP  \"csh -c '$CA'\" > /dev/null"  > command2.sh
source command2.sh

#Start the compute node 3
CA=`echo "$setenvstring cd $PROJ_PATH && nohup $NODE_COMMAND_3 & "`
echo "ssh -t -n -f ramkr004@$NODE_3_IP  \"csh -c '$CA'\" > /dev/null"  > command3.sh
source command3.sh
...
echo "ssh -t -n -f ramkr004@$SERVER_IP  \"csh -c '$CA'\" > /dev/null"  > command4.sh
source command4.sh
...
echo "ssh -t -n -f ramkr004@$CLIENT_IP  \"csh -c '$CA'\" > /dev/null"  > command5.sh
source command5.sh

##### Assign Points ####
echo "ssh -t -n -f ramkr004@$SERVER_IP  ' ps -ef  > log_0  ' "  > grade0.sh
echo "ssh -t -n -f ramkr004@$CLIENT_IP  ' ps -ef > log_1  ' "  > grade1.sh
echo "ssh -t -n -f ramkr004@$NODE_0_IP  ' ps -ef  > log_2  ' "  > grade2.sh
echo "ssh -t -n -f ramkr004@$NODE_1_IP  ' ps -ef  > log_3  ' "  > grade3.sh
echo "ssh -t -n -f ramkr004@$NODE_2_IP  ' ps -ef > log_4  ' "  > grade4.sh
echo "ssh -t -n -f ramkr004@$NODE_3_IP  ' ps -ef > log_5  ' "  > grade5.sh
